...
To add a node use the Nodes palette, the toolbar's Add node, or right-click the canvas. **Add node → 📝 Note** drops a free-text annotation block (double-click to edit; resizable) — notes are saved with the workflow but ignored by the engine. Drag from a node's `out` pin to another node's `in` pin to connect; a node grows extra input pins as you connect more parents (joins). The canvas pans/zooms with the mouse and is HiDPI-aware. The bottom panel has two tabs — an interactive **Shell** (a real PTY into the server) and a **Log** stream of run/listener output — toggled from the toolbar. The Executions page lists runs; click one for step inputs/outputs/error detail (Display nodes render their content here). You can also drag a workflow `.json` / `.yaml` (imports as a new workflow) or a `.nbtpack`/`.zip` (installs as a package) anywhere onto the window. The Workflows sidebar can **export all workflows** (header download button) or **a single folder** (download icon on the folder header) as a `.zip` of importable `<folder>/<name>.json|.yaml` files — handy for backing flows up to a git repo.

Workflow files carry the flow's **name** and **folder** alongside the graph, so importing recreates the flow with the right name (file name is only a fallback) and drops it into the right folder. JSON and YAML are interchangeable — see `docs/FLOW_FILES.md` for the file syntax.
...
`packages/nbt-whatsapp-nodes/` adds a **WhatsApp Send** node that sends a message through **WhatsApp Web** (Selenium-driven Chrome, persistent QR login — no Business API). It also works as a standalone CLI (`python packages/nbt-whatsapp-nodes/whatsapp_nodes.py --to <number> --message "hi"`). See that folder's README; it needs `selenium` (installed with the package) and Chrome on the server machine.

`packages/nbt-teams-nodes/` adds a **Teams Send** node that posts to a **Microsoft Teams** channel via an **Incoming Webhook** (a plain HTTPS POST — no Graph API or app registration). Supports the classic MessageCard and the newer Workflows Adaptive Card formats, plus a raw-JSON escape hatch (also usable for Slack/Discord webhooks), and ships a standalone CLI. Store the webhook URL as an environment variable and pass `{{ teams_webhook }}`.

`packages/nbt-youtube-nodes/` adds **YouTube** nodes over the YouTube Data API v3 (via `requests`, no Google client libs): **YouTube Stats** (views/likes/comments — API key), **YouTube Update Video** (title/description), **YouTube Comment** (add or edit), and **YouTube Rate** (like/dislike). Write actions use an OAuth access token or auto-refresh from `yt_client_id`/`yt_client_secret`/`yt_refresh_token` environment variables. See that folder's README for getting the key/token.
